...
// Get safari posts
$query = "SELECT * FROM admin_posts WHERE category = 'safari' ORDER BY created_at DESC";
$stmt = $db->prepare($query);
$stmt->execute();
$safaris = $stmt->fetchAll(PDO::FETCH_ASSOC);
$safari_ids = array_column($safaris, 'id');
$safari_images = fetchPostImages($db, $safari_ids);

// Handle post deletion 
if ($_POST && isset($_POST['delete_post'])) { 
    $post_id = $_POST['post_id']; 
    $delete_query = "UPDATE admin_posts SET deleted = 1 WHERE id = ?"; 
    $delete_stmt = $db->prepare($delete_query); 
    if ($delete_stmt->execute([$post_id])) { 
        $success = "Post moved to recycle bin successfully!"; 
        // Refresh the page to show updated list 
        header("Location: safari.php?page=" . $current_page); 
        exit(); 
    } else { 
...
if ($_POST && isset($_POST['delete_multiple'])) { 
    if (isset($_POST['selected_posts']) && !empty($_POST['selected_posts'])) { 
        ...
        $delete_multiple_query = "UPDATE admin_posts SET deleted = 1 WHERE id IN ($placeholders)"; 
        $delete_multiple_stmt = $db->prepare($delete_multiple_query); 
        if ($delete_multiple_stmt->execute($selected_posts)) { 
            $success = count($selected_posts) . " posts moved to recycle bin successfully!"; 
            header("Location: safari.php?page=" . $current_page); 
            exit(); 
        } else { 
...
<head>
    ...
    <title>Safari Tours - Nakupenda Tours & Safaris</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Navigation Styles - Same as home.php */
        .responsive-navbar {
            position: fixed;
            top: 0;
            left: 0;
...
.cta-button.secondary:hover {
    ...
    color: #ff8800;
        ...
        .safari-card:hover .card-image-stack img {
...
<body>
    ...
    <nav class="responsive-navbar">
        <div class="navbar-container">
            ...
            <ul class="navbar-menu">
                ...
                <li><a href="safari.php" class="active">Safari Tours</a></li>
    ...
    <section class="hero-section">
        <div class="hero-content">
            <h1>Wild Safari Adventures</h1>
            <p>Experience the ultimate African safari with expert guides and luxury accommodations</p>
            <a href="#safari-packages" class="cta-button">Explore Safari Packages</a>
        </div>
    </section>

    <!-- Safari Packages Section -->
    <section class="safari-section" id="safari-packages">
        <h2 class="section-title">Safari Packages</h2>
        
        <div class="search-filter-container">
            <div class="search-box">
                <input type="text" id="searchInput" class="search-input" placeholder="Search safaris by title or description...">
            </div>
        ...
        <div class="safari-grid" id="safari-container">
            <?php foreach ($safaris as $safari): ?>
                <?php
                    $images = $safari_images[$safari['id']] ?? [];
                    if (empty($images) && !empty($safari['image_path'])) {
                        $images = [$safari['image_path']];
                    }
                    $primary_image = $images[0] ?? '';
                ?>
                <div class="safari-card" data-region="<?php echo $safari['region']; ?>" data-title="<?php echo strtolower($safari['title']); ?>" data-description="<?php echo strtolower($safari['description']); ?>">
                    <div class="card-image-stack">
                        <?php foreach ($images as $image): ?>
                            <img src="<?php echo htmlspecialchars($image, ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars($safari['title'], ENT_QUOTES, 'UTF-8'); ?>" loading="lazy">
                        <?php endforeach; ?>
                    </div>
                    <div class="safari-content">
                        <div class="safari-badge">Popular</div>
                        <span class="safari-region"><?php echo ucfirst($safari['region']); ?></span>
                        <h3 class="safari-title"><?php echo $safari['title']; ?></h3>
                        <div class="safari-price" style="font-weight:bold;color:#ffb300;margin-bottom:0.5rem;">
                            <?php if(isset($safari['price']) && $safari['price'] !== null): ?>
                                $<?php echo number_format($safari['price'], 2); ?>
                            <?php endif; ?>
                        </div>
                        <p class="safari-description"><?php echo substr($safari['description'], 0, 150) . '...'; ?></p>
                       
                        <button class="view-details-btn"
                                data-post-id="<?php echo (int)$safari['id']; ?>"
                                data-title="<?php echo htmlspecialchars($safari['title'], ENT_QUOTES, 'UTF-8'); ?>"
                                data-category="safari"
                                data-region="<?php echo htmlspecialchars($safari['region'], ENT_QUOTES, 'UTF-8'); ?>"
                                onclick="trackViewDetailsClick(this); openModal('<?php echo addslashes($safari['title']); ?>', '<?php echo addslashes($safari['description']); ?>', '<?php echo addslashes($primary_image); ?>', '<?php echo $safari['region']; ?>')">
                            View Details
                        </button>
                    </div>
                </div>
    ...
    <script>
        ...
        function openModal(title, description, image, region) {
            ...
            document.getElementById('safariModal').style.display = 'flex';
            document.body.style.overflow = 'hidden';
        }
        
        function closeModal() {
            document.getElementById('safariModal').style.display = 'none';
