...
$count_query = "SELECT COUNT(*) as total FROM admin_posts WHERE deleted = 0"; 
...
$category_filter = ''; 
$region_filter = ''; 
$query = "SELECT * FROM admin_posts WHERE deleted = 0"; 
$params = []; 
...
if ($_POST && isset($_POST['delete_post'])) { 
    ...
    if ($delete_stmt->execute([$post_id])) { 
        ...
        header("Location: manageposts.php?page=" . $current_page); 
        exit(); 
    } else { 
        ...
        logSystemActivity(
            $db,
            'content',
            'post_soft_delete_failed',
            'Failed moving post to recycle bin.',
            'admin',
            $actor_admin_id,
            $actor_admin_name,
            'post',
    ...
    if (isset($_POST['selected_posts']) && !empty($_POST['selected_posts'])) { 
        ...
        if ($delete_multiple_stmt->execute($selected_posts)) { 
            logSystemActivity(
                $db,
                'content',
                'post_soft_delete_bulk',
                'Multiple posts moved to recycle bin.',
                'admin',
                $actor_admin_id,
                $actor_admin_name,
                'post',
                'bulk',
                ['post_ids' => array_values($selected_posts), 'count' => count($selected_posts)]
            );
            ...
            logSystemActivity(
                $db,
                'content',
                'post_soft_delete_bulk_failed',
                'Failed moving multiple posts to recycle bin.',
                'admin',
                $actor_admin_id,
                $actor_admin_name,
                'post',
...
<head>
    ...
    <title>Manage Posts - Nakupenda Tours</title>
    <style>
    ...
    <div class="dashboard-container">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="manageposts.php" class="active"><span class="menu-icon">📋</span> Manage Posts</a></li>
        ...
        <div class="main-content">
            <div class="header">
                <div class="welcome">
                    <h2>Manage Posts</h2>
                    <p>View, edit, and delete your posts</p>
                </div>
            </div>
            ...
            <form method="POST" id="bulkForm">
                <div class="bulk-actions">
                    <div class="bulk-select">
                        ...
                        <span class="pagination-info">Showing <?php echo count($posts); ?> of <?php echo $total_posts; ?> posts</span>
                    </div>
                    <button type="submit" name="delete_multiple" class="delete-multiple-btn" onclick="return confirm('Are you sure you want to move selected posts to recycle bin?')">Delete Selected</button>
                </div>
                            ...
                            <tbody>
                                <?php foreach ($posts as $post): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php echo isset($post['price']) ? number_format($post['price'], 2) : '-'; ?>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                <a href="editpost.php?id=<?php echo $post['id']; ?>" class="btn-edit" title="Edit Post">✏️</a>
                                                <form method="POST" style="display: inline;">
                                                    <input type="hidden" name="post_id" value="<?php echo $post['id']; ?>">
                                                    <button type="submit" name="delete_post" class="btn-delete" title="Delete Post" onclick="return confirm('Are you sure you want to move this post to recycle bin?')">🗑️</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
