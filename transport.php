...
// Get transport posts
$query = "SELECT * FROM admin_posts WHERE category = 'transport' ORDER BY created_at DESC";
$stmt = $db->prepare($query);
$stmt->execute();
$transports = $stmt->fetchAll(PDO::FETCH_ASSOC);
$transport_ids = array_column($transports, 'id');
$transport_images = fetchPostImages($db, $transport_ids);
?>
...
<head>
    ...
    <title>Transport Services - Nakupenda Tours & Safaris</title>
    <style>
...
.cta-button.secondary:hover {
    ...
    color: #ff8800;
        ...
        .transport-card:hover .card-image-stack img {
...
<body>
    ...
    <nav class="responsive-navbar">
        <div class="navbar-container">
            ...
            <ul class="navbar-menu">
                ...
                <li><a href="transport.php" class="active">Transport</a></li>
    ...
    <section class="hero-section">
        <div class="hero-content">
            ...
            <p>Travel in comfort and style with our fleet of modern vehicles</p>
            <a href="#transport-packages" class="cta-button">Explore Transport services</a>
        </div>
    </section>

    <!-- Transport Section -->
    <section class="transport-section" id="transport-packages">
        <h2 class="section-title">Our Vehicle Fleet</h2>
        
        <div class="search-filter-container">
            <div class="search-box">
                <input type="text" id="searchInput" class="search-input" placeholder="Search vehicles by title or description...">
            </div>
        ...
        <div class="transport-grid" id="transport-container">
            <?php foreach ($transports as $transport): ?>
                <?php
                    $images = $transport_images[$transport['id']] ?? [];
                    if (empty($images) && !empty($transport['image_path'])) {
                        $images = [$transport['image_path']];
                    }
                    $primary_image = $images[0] ?? '';
                ?>
                <div class="transport-card" data-region="<?php echo $transport['region']; ?>" data-title="<?php echo strtolower($transport['title']); ?>" data-description="<?php echo strtolower($transport['description']); ?>">
                    <div class="card-image-stack">
                        <?php foreach ($images as $image): ?>
                            <img src="<?php echo htmlspecialchars($image, ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars($transport['title'], ENT_QUOTES, 'UTF-8'); ?>" loading="lazy">
                        <?php endforeach; ?>
                    </div>
                    <div class="transport-content">
                        <div class="transport-badge">Available</div>
                        <span class="transport-region"><?php echo ucfirst($transport['region']); ?></span>
                        <h3 class="transport-title"><?php echo $transport['title']; ?></h3>
                        <div class="transport-price" style="font-weight:bold;color:#ffb300;margin-bottom:0.5rem;">
                            <?php if(isset($transport['price']) && $transport['price'] !== null): ?>
                                $<?php echo number_format($transport['price'], 2); ?>
                            <?php endif; ?>
                        </div>
                        <p class="transport-description"><?php echo substr($transport['description'], 0, 120) . '...'; ?></p>
                       
                        <button class="view-details-btn"
                                data-post-id="<?php echo (int)$transport['id']; ?>"
                                data-title="<?php echo htmlspecialchars($transport['title'], ENT_QUOTES, 'UTF-8'); ?>"
                                data-category="transport"
                                data-region="<?php echo htmlspecialchars($transport['region'], ENT_QUOTES, 'UTF-8'); ?>"
                                onclick="trackViewDetailsClick(this); openModal('<?php echo addslashes($transport['title']); ?>', '<?php echo addslashes($transport['description']); ?>', '<?php echo addslashes($primary_image); ?>', '<?php echo $transport['region']; ?>')">
                            View Details
                        </button>
                    </div>
                </div>
